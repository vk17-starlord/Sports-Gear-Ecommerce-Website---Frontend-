...
<p className='text-left text-xl'>Enter Your Email</p>
  <input type="email" onChange={(e)=>setEmail(e.target.value)} name="email" class="mt-1 px-3 py-2 bg-white border shadow-sm border-slate-300 placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-sky-500 block w-full rounded-md sm:text-sm focus:ring-1" placeholder="JohnDoe@example.com" />   
 </div>            
 
 <div className="input mt-[25px]">
<p className='text-left text-xl'>Enter Your Password</p>
  <input type="password" name="password" onChange={(e)=>setPassword(e.target.value)} class="mt-1 px-3 py-2 bg-white border shadow-sm border-slate-300 placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-sky-500 block w-full rounded-md sm:text-sm focus:ring-1" placeholder="yourpass@1234" />   
 </div>            
...
<p>Already Have a account ? <span className='text-[#405DF8] text-semibold'><Link to='/SignUp'>Sign Up </Link></span></p>
 {
   msg ? 
   <div class="mt-[4vh] bg-[#FFF3F9] text-[#F1002B] w-[max-content] mx-auto px-[30px]  py-[12px] rounded-md  drop-shadow-[.2px_.2px_rgba(0,0,0,0.25)]">Please Enter Valid Details <i className='text-[#F1002B] align-middle text-xl px-2 bx bxs-error-alt'></i></div>
        :null    
 }
 </div>
        </div>
